...
void readFromFile()
{
	...
	IOLibrary::print("Reading from file: '" + test_filename + "':");
...
void openAnotherFileSameStream()
{
	...
	if(fstream)
		IOLibrary::print("Opened a new file for an existing stream. File: '" + another_filename + "'.");
	else
		IOLibrary::print("Something wrong with the new file.");
...
void alreadyOpenedFile()
{
	...
	if(fstream.fail())
	{
		IOLibrary::print("Tried to call open for a stream which already has an opened file. Failbit is set.");
		fstream.clear();
		IOLibrary::print("Cleared the failbit for the stream.");
	}
}
...
void file_basics_main() 
{
	...
	// Read from file and IOLibrary::print its contents line by line
	readFromFile();
